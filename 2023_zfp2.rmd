...
```{r}
areas <- read_tsv("/data/2023_zfp2/Silique_imaging/Wiesner/2023-09_Wiesner/repulum_areas.tsv")

areas_tot <- areas |> 
  group_by(genotype, replicate, daf) |> 
  summarise(total_area = mean(total_area, na.rm = TRUE))

rep_plot <- ggplot(areas,
                   aes(
                     x = daf,
                     y = area
                   )) +
  ...
  labs(
    x = "DAF",
    y = "Repulum area [px; transverse section]"
  ) +
  theme_leo() +
  theme(
...
tot_plot <- ggplot(areas_tot,
  ...
  labs(
    x = "DAF",
    y = "Total silique area [px; transverse section]"
  ) +
  theme_leo() +
  theme(
    legend.position = c(0.1, 0.9),
